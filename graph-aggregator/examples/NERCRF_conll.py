    ...
    for k in range(len(lines)):
        ...
        if len(line) == 0 or line.startswith('-DOCSTART-'): # new sentence or new document
            if len(curr_words) > 0:
                ...
                true_tags.append(curr_ttags)
                pred_tags.append(curr_ptags)
                curr_words = list()
                curr_ttags = list()
                curr_ptags = list()
            continue
        ...
        ttag = strings[1].strip()  # be default, we take the last tag
        ptag = strings[2].strip()  # be default, we take the last tag
        curr_words.append(word)
        curr_ttags.append(ttag)
        curr_ptags.append(ptag)
        if k == len(lines) - 1:
            words.append(curr_words)
            true_tags.append(curr_ttags)
            pred_tags.append(curr_ptags)
...
def evaluate_tokenacc(output_file):
    words, true_tags, pred_tags = read_data(output_file, mode=' ')
    cnt = 0
    match = 0
    for target_seq, output_seq in zip(true_tags, pred_tags):
...
def evaluate(output_file, score_file, evaluate_raw_format=False, o_tag='O'):
    if evaluate_raw_format:
        os.system("./examples/eval/conlleval -r -o %s < %s > %s" %
                  (o_tag, output_file, score_file))
    else:
        os.system("./examples/eval/conlleval -o %s < %s > %s" %
                  (o_tag, output_file, score_file))
