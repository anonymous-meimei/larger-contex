...
from src.layers.layer_transformer import  Transformer_Layer
...
class TaggerBiRNNCNN(TaggerBase):
    ...
    def __init__(self,args, word_seq_indexer, tag_seq_indexer, class_num, batch_size=1, rnn_hidden_dim=100,
        ...
        if args.if_elmo:
            self.elmo_embeddings_layer = LayerElmoEmbeddings(args, gpu, args.options_file, args.weight_file,
    ...
    def forward(self, word_sequences, input_sequence):
        z = torch.tensor([0])
        ...
        # if self.args.if_elmo == True:
        #     z_flair = self.elmo_embeddings_layer(word_sequences)
        #     z = self.dropout(z_flair)
        #     print('using elmo emb...')
        ...
        # if self.if_word == True and self.args.if_bert == True:
        #     z_word_embed = self.word_embeddings_layer(word_sequences)
        #     z_word_embed_d = self.dropout(z_word_embed)
        #     z_bert = self.bert_embeddings_layer(word_sequences)
        ...
        #     z_word_embed = self.word_embeddings_layer(word_sequences)
        #     z_word_embed_d = self.dropout(z_word_embed)
        #     z_flair = self.bert_embeddings_layer(word_sequences)
        #     z_flair_d = self.dropout(z_flair)
        #     z = torch.cat((z_word_embed_d, z_flair_d), dim=2)
        # if self.args.if_elmo == True and self.args.if_bert == True:
        ...
        if self.args.transformer:
            ...
            if not self.args.transformer_useSentEncode:
                z_rnn_out = self.apply_mask(self.transClassify_lin(z),mask)
    ...
    def get_loss(self, word_sequences_train_batch,input_sequences_train_batch,  tag_sequences_train_batch):
        outputs_tensor_train_batch_one_hot,pred = self.forward(word_sequences_train_batch,input_sequences_train_batch)
    ...
    def get_prob(self,word_sequences_train_batch,input_bert_train_batch,tag_sequences_train_batch):
        outputs_tensor_train_batch_one_hot, pred = self.forward(word_sequences_train_batch,input_bert_train_batch, )
    ...
    def predict_idx_from_words(self, word_sequences,input_sequences):
        self.eval()
        outputs_tensor, _ = self.forward(word_sequences,input_sequences)  # batch_size x num_class+1 x max_seq_len
    ...
    def predict_tags_from_words(self, word_sequences,input_sequences, batch_size=-1):
        if batch_size == -1:
        ...
        for n in range(batch_num):
            ...
            curr_output_idx = self.predict_idx_from_words(word_sequences[i:j],input_sequences[i:j])
